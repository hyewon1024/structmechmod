...
def get_real_lagrangian_metrix_cartpole(env, state, action):
...
def normalize(data):
    obs_array, next_obs_array = data[0], data[2]
    mean, std = obs_array.mean(axis=0), obs_array.std(axis=0)
    obs_array = (obs_array - mean) / std
    next_obs_array = (next_obs_array - mean) / std
    return (obs_array,) + data[1:2] + (next_obs_array,) + data[3:]
...
if __name__ == "__main__":
    ...
    train_data = generate_cartpole_data(env, num_samples=100, save=False)[:-2]
    obs_array, next_obs_array = train_data[0], train_data[2]
    obs_array[:, [1, 2]], next_obs_array[:, [1, 2]] = obs_array[:, [2, 1]], next_obs_array[:, [2, 1]]
    train_data = (obs_array,) + train_data[1:2] + (next_obs_array,) + train_data[3:]

    valid_data = generate_cartpole_data(env, num_samples=30, save=False)[:-2]
    ...
    valid_data = (obs_array_,) + valid_data[1:2] + (next_obs_array_,) + valid_data[3:]
    ...
    trained_params = train(smm, train_data, valid_data, hparams, loss_log_path='train_loss.mat')

    test_num_samples = 200
    test_datasets = generate_cartpole_data(env, num_samples=test_num_samples, save=True)
    test_data, action_data, img_episode = test_datasets[:-2], test_datasets[-2], test_datasets[-1]
    ...
    for epi in range(test_num_samples):
        ...
        mass_matrix_pred, corrioli_matrix_pred, gravitational_term_pred, generalized_force_pred = get_real_lagrangian_metrix_cartpole(env, states, action_data[epi])
