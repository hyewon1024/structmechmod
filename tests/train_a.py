...
def setup_and_train_model(train_data, valid_data, hidden_sizes, thetamask, hparams,  loss_log_path):
    ...
    trained_params = train(smm, train_data, valid_data, hparams, loss_log_path)
    return smm, trained_params

def evaluate_model(env, smm, test_data, action_data,output_path):
...
if __name__ == "__main__":
    ...
    smm, _ = setup_and_train_model(train_data, valid_data, hidden_sizes, thetamask, hparams, "acrobot.mat")

    # Evaluation
    test_datasets= generate_rand_data(env, ntrajs, traj_len, dt, maxu=maxu, stddev=stddev)
    test_data, action_data = test_datasets[:-1], test_datasets[-1]
    output_path = os.path.join(metric_folder, 'metrics_data.mat')

    evaluate_model(env, smm, test_data, action_data, output_path)
    env.close()